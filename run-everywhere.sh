...
# Description
## Script that runs commands on several systems on local network.
...
# Remove options and leave remaining args
shift "$(( OPTIND -1 ))"
...
# Set exit status to 0 and overwrite with default script exit status
EXIT_STATUS='0'
...
do
  ...
  # If it's a dry run, don't execute anything jsut echo it.
  if [[ "${DRY_RUN}" = 'true' ]]
  then
